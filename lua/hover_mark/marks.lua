...
function Mark:delete_mark(mark, clear)
    ...
    -- adjust for lowest_available_mark if it is lowercase
...
function Mark:delete_line_marks()
    ...
    -- delete mark modifiers the table so make a copy
...
function Mark:next_mark()
    ...
    local next = utils.serach(marks, {line=line}, {line=math.huge}, comparator, self.opt.cyclic)
...
function Mark:prev_mark()
    ...
    local prev = utils.serach(marks, {line=line}, {line=-1}, comparator, self.opt.cyclic)
...
function Mark:buffer_to_list(list_type, bufnr)
    ...
    for mark, data in pairs(self.buffers[bufnr].placed_marks) do
        ...
        table.insert(items, { bufnr = bufnr, lnum = data.line, col = data.col + 1, text = "mark" .. mark .. ": " .. text})
    end
...
function Mark:refresh(bufnr, force)
    ...
    for _, data in ipairs(vim.fn.getmarklist()) do
        ...
        if utils.is_lower(mark) and pos[1] == bufnr and (force or not cached_mark or pos[2] ~= cached_mark.line) then
            self:register_mark(mark, pos[2], pos[3], bufnr)
        end
    end
end
