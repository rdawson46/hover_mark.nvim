...
function M:set()
    local err, input = pcall(function() return string.char(vim.fn.getchar())end)
    if not err then
        return
    end
...
function M.set_mark()
    if not M.excluded_fts[vim.bo.ft] then
        M.mark_state:place_mark_cursor()
    end
end
...
function M.toggle_signs(bufnr)
    if not bufnr then
        ...
        for buf, _ in pairs(M.mark_state.opt.buf_signs) do
            M.mark_state.otp.buf_signs[buf] = M.mark_state.opt.signs
        end

        for buf, _ in pairs(M.bookmark_state.opt.buf_signs) do
            M.bookmark_state.otp.buf_signs[buf] = M.bookmark_state.opt.signs
        end
    else
        M.mark_state.opt.buf_signs[bufnr] = not utils.option_nil(
        M.mark_state.opt.buf_signs[bufnr], M.mark_state.otp.signs)
...
local function apply_mappings()
    for cmd, key in pairs(M.mappings) do
        vim.cmd('nnoremap <silent> '..key.." <cmd>lua require'hover_mark'."..cmd.."()<cr>")
    end
end
...
local function setup_autocommands()
    ...
    autocmd!
    autocmd BufEnter * lua require'hover_mark'.refresh(true)
    autocmd BufDelete * lua require'hover_mark'._on_delete()
    augroup end]]
end
